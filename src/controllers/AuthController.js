...
class AuthController {
  ...
  async signup({ protocol, headers, body }, res, next) {
    try {
      ...
      newUser.token = token;
      const link = `${protocol}//${headers.host}/api/v1/auth/confirmEmail?token=${token}&id=${newUser.uuid}`;
      await sendEmail(
        email,
  ...
  */
  async sendResetLink(req, res) {
    ...
    if (!inValidEmail(email)) {
      ...
      const link = `${req.protocol}//${req.headers.host}/api/v1/auth/resetPassword/${uuid}/${token}`;
      try {
        await sendEmail(
          email,
